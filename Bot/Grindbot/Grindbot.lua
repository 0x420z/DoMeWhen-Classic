...
function Grindbot:SwapMode()
    ...
    if not DMW.Player.Swimming and not DMW.Player.Combat and not DMW.Player:Standing() and (DMW.Player.PetHappiness < 3 or DMW.Player.HP < 95 and Eating or UnitPower('player', 0) > 0 and (UnitPower('player', 0) / UnitPowerMax('player', 0) * 100) < 95 and Drinking) then
    ...
    if not DMW.Player.Swimming and not DMW.Player.Combat and not IsMounted() and (DMW.Player.PetHappiness < 3 or DMW.Player.HP < Settings.RestHP or UnitPower('player', 0) > 0 and (UnitPower('player', 0) / UnitPowerMax('player', 0) * 100) < Settings.RestMana) then
        Grindbot.Mode = Modes.Resting
        return
    end
